...
if __name__ == "__main__":
    ...
    eval_stg1, hyperparams = load_model(args.model1, env, ts=args.checkpoint)
    ...
    with torch.no_grad():
        ...
        for scene in tqdm.tqdm(scenes):
            ...
            predictions1, features1 = eval_stg1.predict(
            ...
            predictions2, features2 = eval_stg2.predict(
                scene, timesteps, ph, min_history_timesteps=prediction_parameters[0],
                min_future_timesteps=prediction_parameters[1],
                selected_node_type=args.node_type)
        ...
        pd.DataFrame({'value': eval_ade_batch_errors, 'metric': 'ade', 'type': 'full'}
                     ).to_csv(os.path.join('./', 'eth_ewta500' + '_ade_full.csv'))
        pd.DataFrame({'value': eval_fde_batch_errors, 'metric': 'fde', 'type': 'full'}
                     ).to_csv(os.path.join('./', 'eth_ewta500' + '_fde_full.csv'))